...
def determine_attack(handshake_attempts, login_attempts):
    ...
    # Check for repeated TCP handshake attempts
    if len(handshake_attempts) > 1:
        return "Potential TCP SYN Flood Attack"
    # Check for repeated login attempts
    elif len(login_attempts) > 1:
...
def analyze_packets(pcap_file):
    ...
    for packet_number, packet in enumerate(pyshark.FileCapture(pcap_file, display_filter=filter_expression), 1):
        if 'TCP' in packet:
            ...
            if packet.tcp.flags_syn == '1' and packet.tcp.flags_ack == '0':
                ...
                handshake_attempts[handshake_key] = handshake_attempts.get(handshake_key, []) + [(packet_number, packet)]
            else:
                username, password = extract_credentials(packet)
                if username and password:
                    login_key = (src_ip, dst_ip, src_mac, dst_mac, username, password)
                    login_attempts[login_key] = login_attempts.get(login_key, []) + [(packet_number, packet)]
                shell_command = extract_shell_command(packet)
                if shell_command:
                    shell_commands.append((packet_number, src_ip, dst_ip, src_mac, dst_mac, shell_command))
    ...
    for key, packets in handshake_attempts.items():
        if len(packets) > 1:
            ...
            for packet_number, packet in packets:
                print(f"  Packet Number: {packet_number}, Protocol: TCP")
                print("-" * 50)
    ...
    if shell_commands:
        ...
        for packet_number, src_ip, dst_ip, src_mac, dst_mac, command in shell_commands:
            print(f"  Packet Number: {packet_number}, Source IP: {src_ip}, Destination IP: {dst_ip}")
